...
export const TournamentView = () => {
  ...
  const getLeagueName = async (id: number) => {
    const { data } = await axios.get(
  ...
  const { data: leagueData } = useQuery({
    ...
    queryFn: () => getLeagueName(Number(tournamentId)),
  });
    ...
    <>
      <Flex width="100%" alignItems="center" justifyContent="space-around">
        ...
        <Flex justifySelf="center">
          <Heading justifySelf="center">{leagueData}</Heading>
        </Flex>
        <Flex> </Flex>
      </Flex>
                ...
                <Link to={`/view/match/${match.id}`} className="match">
                  <Flex className="match">
                    ...
                    <Flex
                      ...
                      gap={8}
                      fontWeight={
                        match.winner === match.away_team_name
                          ? "bold"
                          : "normal"
                      }
                      bg={
